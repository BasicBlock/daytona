...
//
//	@id				Initialize
func (s *server) Initialize(otelServiceName string, entrypointLogFilePath string, snapshot *string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		err := s.initTelemetry(ctx.Request.Context(), otelServiceName, entrypointLogFilePath, snapshot)
		if err != nil {
			ctx.AbortWithError(http.StatusBadRequest, err)
			return
		}
