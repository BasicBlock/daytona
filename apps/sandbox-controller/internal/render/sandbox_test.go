...
func TestPodDefaultsToGvisorWorkloadOnly(t *testing.T) {
	...
	if len(pod.Spec.Containers) != 1 {
		t.Fatalf("expected only workload container, got %d", len(pod.Spec.Containers))
	}
	if pod.Spec.Containers[0].Name != WorkloadContainerName {
		t.Fatalf("expected workload container, got %s", pod.Spec.Containers[0].Name)
	}
